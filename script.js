...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (syncGistBtn) {
        syncGistBtn.addEventListener('click', async () => {
            ...
            const note = dom.noteInput ? dom.noteInput.value : '';
            const data = {
                annotations,
                note,
            ...
            if (!token) {
                token = await new Promise(resolve => {
                    ...
                    modal.innerHTML = `<div style="position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.3);z-index:9999;display:flex;align-items:center;justify-content:center;"><div style="background:#fff;padding:24px 32px;border-radius:8px;box-shadow:0 2px 8px #0002;min-width:320px;"><h3 style='margin-top:0'>输入 GitHub Gist Token</h3><input id='gistTokenInput' type='password' style='width:100%;font-size:16px;padding:8px;margin-bottom:12px;' placeholder='Token'><br><button id='gistTokenOk' style='padding:6px 18px;'>确定</button></div></div>`;
                    document.body.appendChild(modal);
                    modal.querySelector('#gistTokenOk').onclick = () => {
                        const val = modal.querySelector('#gistTokenInput').value;
            ...
            // 获取加密密码
            let encPwd = await new Promise(resolve => {
                const modal = document.createElement('div');
                modal.innerHTML = `<div style="position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.3);z-index:9999;display:flex;align-items:center;justify-content:center;"><div style="background:#fff;padding:24px 32px;border-radius:8px;box-shadow:0 2px 8px #0002;min-width:320px;"><h3 style='margin-top:0'>输入加密密码</h3><input id='gistEncPwdInput' type='password' style='width:100%;font-size:16px;padding:8px;margin-bottom:12px;' placeholder='加密密码'><br><button id='gistEncPwdOk' style='padding:6px 18px;'>确定</button></div></div>`;
                document.body.appendChild(modal);
                modal.querySelector('#gistEncPwdOk').onclick = () => {
                    const val = modal.querySelector('#gistEncPwdInput').value;
                    document.body.removeChild(modal);
                    resolve(val);
                };
            });
            ...
            // 加密数据
            const encrypted = window.CryptoJS ? window.CryptoJS.AES.encrypt(JSON.stringify(data), encPwd).toString() : JSON.stringify(data);
            // 显示同步进度
            const syncModal = document.createElement('div');
            syncModal.innerHTML = `<div style="position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.15);z-index:9999;display:flex;align-items:center;justify-content:center;"><div style="background:#fff;padding:24px 32px;border-radius:8px;box-shadow:0 2px 8px #0002;min-width:320px;text-align:center;"><span id='syncStatus'>正在同步到Gist...</span></div></div>`;
            ...
            try {
                const res = await fetch('https://api.github.com/gists', {
                    ...
                    body: JSON.stringify({
                        description: 'AI LearnEnglish 阅读标注数据（加密）',
                        public: false,
                        files: {
                            'annotations.enc': {
                                content: encrypted
                ...
                if (gist.html_url) {
                    ...
                    resultModal.innerHTML = `<div style="position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.15);z-index:9999;display:flex;align-items:center;justify-content:center;"><div style="background:#fff;padding:24px 32px;border-radius:8px;box-shadow:0 2px 8px #0002;min-width:320px;text-align:center;"><div style='margin-bottom:12px;'>同步成功！</div><input id='gistUrlCopy' value='${gist.html_url}' style='width:90%;font-size:15px;padding:6px 8px;margin-bottom:8px;' readonly><br><button id='copyGistUrlBtn' style='padding:6px 18px;'>复制地址</button> <button id='closeGistResultBtn' style='padding:6px 18px;'>关闭</button></div></div>`;
                    document.body.appendChild(resultModal);
                    resultModal.querySelector('#copyGistUrlBtn').onclick = () => {
                        const input = resultModal.querySelector('#gistUrlCopy');
                        input.select();
                        document.execCommand('copy');
                    };
                    resultModal.querySelector('#closeGistResultBtn').onclick = () => {
                        document.body.removeChild(resultModal);
                    };
                } else {
                    alert('同步失败：' + (gist.message || '未知错误'));
                }
            } catch (e) {
                document.body.removeChild(syncModal);
